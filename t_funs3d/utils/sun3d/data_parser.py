...
import t_funs3d.utils.sun3d.homogenous as hm
from t_funs3d.utils.sun3d.data_parser_paths import data_asset_to_path
from t_funs3d.utils.sun3d.rigid_interpolation import rigid_interp_geodesic, rigid_interp_split
...
class DataParser:
    ...
    def get_mask_index(self, visit_id: str, mask_type: str, exp_dir=None) -> dict:
        if exp_dir is None:
            exp_dir = f"{self.root}/{self.split}"
            
        with open(
            os.path.join(
                exp_dir, f"{visit_id}_{mask_type}_masks.json"
                # "/workspace/t-funs3d_outputs/eval/modified_preprocess/07/association", f"{visit_id}_{mask_type}_masks.json"
            )
        ) as f:
            data = json.load(f)
        return data
    ...
    def get_mask_features(self, visit_id: str, video_id: str, exp_dir=None) -> np.ndarray:
        """
        Retrieve the mask features for a specific visit and video.
        """
        if exp_dir is None:
            exp_dir = f"{self.root}/{self.split}"
        # exp_dir = "/workspace/t-funs3d_outputs/eval/modified_preprocess/07/mask_features"
        mask_features = np.load(
            os.path.join(exp_dir, visit_id, f"{visit_id}_{video_id}_features.npy")
        )
        return mask_features
    
    def get_masks_frames_association(self, visit_id: str,  video_id: str, exp_dir=None) -> dict:
        """
        Retrieve the mask to frame association for a specific visit and video.
        """
        if exp_dir is None:
            exp_dir = f"{self.root}/{self.split}"
        # exp_dir = "/workspace/t-funs3d_outputs/eval/modified_preprocess/07/mask_features"
        with open(
            os.path.join(exp_dir, visit_id, f"{visit_id}_{video_id}_association.json")
        ) as f:
            mask_frame_association = json.load(f)

        return mask_frame_association
