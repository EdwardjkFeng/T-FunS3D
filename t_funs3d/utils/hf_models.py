...
def process_sam_prompts(
    ...
    if n_points > batch:
        ...
        for i_b in range(NB):
            ...
            mask_sizes = masks_i.sum(dim=(2, 3))
            # print(mask_sizes)
            top_scores_idxs = torch.argmin(mask_sizes, dim=1).to(masks_i.device)
            # print(masks_i.shape, top_scores_idxs)
        ...
        mask_sizes = masks.sum(dim=(2, 3))
        # print(mask_sizes)
        top_scores_idxs = torch.argmin(mask_sizes, dim=1).to(masks.device)
        # print(masks.shape, top_scores_idxs)
        idxs = torch.arange(0, masks.shape[0]).to(masks.device)
        masks = masks[idxs, top_scores_idxs]

    return masks

def process_sam_bbox_prompts(
    sam_model,
    sam_processor,
    img,
    boxes: List,
    ...
    """
    Return SAM masks by given prompts, given as a list of 2D bounding boxes.
    If all_masks = True, returns all masks hierarchy, otherwise only the first
    """

    # input_points = torch.tensor(boxes).to(sam_model.device)
    inputs = sam_processor(img, input_boxes=boxes, return_tensors="pt").to(
        sam_model.device
    )
    image_embeddings = sam_model.get_image_embeddings(inputs["pixel_values"])
    ...
    inputs.pop("pixel_values", None)
    inputs.update({"image_embeddings": image_embeddings})
    inputs["input_boxes"] = inputs["input_boxes"].unsqueeze(0)
    ...
    if n_boxes > batch:
        ...
        masks = list()
        scores = list()
        NB = math.ceil(n_boxes / batch)
        copied_boxes = inputs["input_boxes"].clone()
        for i_b in range(NB):
            inputs["input_boxes"] = copied_boxes[
                :, i_b * batch : (i_b + 1) * batch, :, :
            ]
            with torch.no_grad():
                outputs = sam_model(**inputs)
...
def init_clip_visual(clip_model=None, download_root=None):
    ...
    if clip_model is None:
        ...
        print(f"CLIP model output dimension: {output_dim}")
    elif "siglip" in clip_model:
...
def init_clip_textual(clip_model=None):
    '''Initialize CLIP models'''
    if clip_model is None:
...
def init_rsam(device: str):
    device = torch.device(device)
    model = "jadechoghari/robustsam-vit-large"  # "facebook/sam-vit-huge"
    sam_model = SamModel.from_pretrained(model).to(device)
    sam_processor = SamProcessor.from_pretrained(model)

    return sam_model, sam_processor

def process_sam_points(
    sam_model,
    sam_processor,
    image,
    point_coords,
    ...
    for i in range(num_random_rounds):
        ...
        inputs = sam_processor(
            image,
            input_points=[point_coords_new[:num_selected_points].tolist()],
            return_tensors="pt",
        ).to(sam_model.device)

        if image_embeddings is None: # Initialize image embeddings once before inference
            # https://discuss.huggingface.co/t/two-questions-about-segment-anything-model-sam-in-transformers/63825/4
            image_embeddings = sam_model.get_image_embeddings(inputs.pixel_values)

        del inputs["pixel_values"]
        with torch.no_grad():
            outputs = sam_model(image_embeddings=image_embeddings, **inputs)
        masks = sam_processor.image_processor.post_process_masks(
            outputs.pred_masks.cpu(),
            inputs["original_sizes"].cpu(),
            inputs["reshaped_input_sizes"].cpu(),
