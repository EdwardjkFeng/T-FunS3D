...
from t_funs3d.utils.misc import np_normalize, sort_alphanumeric, args2dict
from t_funs3d.utils.sun3d.data_parser import DataParser
from t_funs3d.utils.sun3d.fusion_util import PointCloudToImageMapper
...
def post_process_pcd(
    ...
    for i, (frame_id, video_id, depth_path, intrinsic, pose, mask_f) in enumerate(
        ...
        if mask_f.shape != (1920, 1440) and mask_f.shape != (1440, 1920):
            mask_f = np.zeros(depth.shape)
...
def get_prediction(
    ...
    """
    try:
        path = join(exp, frame_folder, f"{visit_id}_{desc_id}.npz")
        data = np.load(path)
