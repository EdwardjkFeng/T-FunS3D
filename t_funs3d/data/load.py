...
class Camera:
    def __init__(self, 
        ...
        # Check if intrinsic path is a file or folder
        if os.path.isfile(self.intrinsic_path):
            self.mode = "global"
            self.global_intrinsic = np.loadtxt(self.intrinsic_path)[:3, :3]
        elif os.path.isdir(self.intrinsic_path):
            self.mode = "per_frame"
        else:
            raise ValueError(f"Invalid intrinsic path {intrinsic_path}.")

    def load_intrinsics(self, idx=None):
        if self.mode == "global":
            return self.global_intrinsic
        elif self.mode == "per_frame":
            if idx is None:
                raise ValueError("Index must be provided for per-frame intrinsics.")
            path = os.path.join(self.intrinsic_path, str(idx) + '.pincam')
            _, _, fx, fy, cx, cy = np.loadtxt(path)
            intrinsic = np.array([
                [fx,    0.0,    cx],
                [0.0,    fy,    cy],
                [0.0,   0.0,   1.0],
    ...
    def get_adapted_intrinsic(self, desired_resolution, idx=None):
        '''Get adjusted camera intrinsics.'''
        intrinsic = self.load_intrinsics(idx)
        if intrinsic is None:
            raise ValueError("Intrinsic matrix could not be loaded.")
    ...
    def load_poses(self, indices):
        path = os.path.join(self.poses_path, str(indices[0]) + '.txt')
